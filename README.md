...
# Data Model
<<img width="1704" height="1428" alt="image" src="https://github.com/user-attachments/assets/33077256-66a9-4c9b-8c1e-35510ff3c2f9" />
...
* **Managerial Justification:** This query returns the name of the class, the ID of the session, the start time, and the attendance count for the different classes offered.
*  **SQL:** `SELECT Classes.class_name, Class_Session.session_id, Class_Session.start_datetime, COUNT(Attendance_Logs.Members_member_id) AS attendance_count FROM Class_Session JOIN Classes ON 
Class_Session.class_id = Classes.class_id LEFT JOIN Attendance_Logs ON Class_Session.session_id = Attendance_Logs.Class_Session_session_id AND Attendance_Logs.status = 'Present'
GROUP BY Classes.class_name, Class_Session.session_id, Class_Session.start_datetime ORDER BY attendance_count DESC;`
